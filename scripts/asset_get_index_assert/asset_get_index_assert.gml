...
/// function will use `asset_get_stripped()` to check if the target asset was stripped from the
...
/// for an asset that doesn't exist anywhere) then this function will return `-1`.
/// 
...
///   asset has been stripped and will return `-1` for an asset it cannot find. If you would like
///   to enable errors, please tick the `Force Export` option in `asset_get_stripped_options`.
/// 
/// - The HTML5 runtime is not supported and this function will never raise an error on when
///   using the HTML5 runtime.
/// 
...
function asset_get_index_assert(_assetName)
{
    ...
    if ((real(_asset) < 0) && asset_get_stripped(_assetName))
    {
        show_error($" \nAsset \"{_assetName}\" has been stripped from your game during compile.\nPlease refer to GameMaker documentation for instructions on resolving this problem.\n ", true);
    }
