...
/// removed during compile due to the "Automatically remove unused assets when compiling" feature.
/// This function will return `true` only if the asset exists in the project file but not at
/// runtime. This means `asset_get_removed()` will return `false` if the asset name is simply
/// misspelled.
/// 
/// The asset name should be passed to this function as a string e.g. you should call
/// `asset_get_removed("obj_example")` for an object named `obj_example`.
/// 
/// Please note:
/// 
...
///   `asset_get_removed()` will always return `false` when running your game outside the IDE.
///   If you'd like to enable `asset_get_removed()` for executable builds, please tick the
///   `Force Export` option in `removed_asset_detector`.
/// 
...
function asset_get_removed(_assetName)
{
    static _projectResourceDict = (function()
    {
        ...
        if ((os_type != os_operagx) && (os_browser != browser_not_a_browser))
        {
            ...
            show_debug_message("Warning! `asset_get_removed()` is not supported when using the HTML5 runtime");
        }
        else
        {
            try
            {
                var _buffer = buffer_load("removed_asset_detector.json");
    ...
    {
        //Asset exists in the project; definitionally not removed
        return false;
    }
    else
    {
        //If we find the asset name in the project JSON then the asset has been removed
        return struct_exists(_projectResourceDict, _assetName);
    }
}

show_debug_message("Welcome to Removed Asset Detector by Juju Adams and Sidorakh! This version 2.0.2, 2025-11-18.");